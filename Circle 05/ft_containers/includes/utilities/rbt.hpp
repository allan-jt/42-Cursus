...
void	print_level_order() {
	...
	while (!q.empty()) {
		...
		q.pop();
		value_type tmp = temp->get_value();
		std::cout << tmp.first << ":" << tmp.second <<
			"(" << temp->is_black() << ")"  << " ";
		if (temp->get_left() && temp->get_left() != _end_node)
			q.push(temp->get_left());
		if (temp->get_right() && temp->get_right() != _end_node)
	...
	std::cout << std::endl;
		...
		void	print_in_order(pointer node) {
			if (!node)
				return;
			print_in_order(node->get_left());
			std::cout << node->get_value().first << " ";
      ...
      std::cout << root->get_value().first << "(" << root->get_color() << ")" << std::endl;
        for (int i = 10; i < space; i++) std::cout << " ";
          if (root->get_parent())
            std::cout << "Parent: " << root->get_parent()->get_value().first << std::endl;
